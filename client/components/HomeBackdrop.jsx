...
export default class Home extends Component {
	...
	componentDidMount() {	
		...
		stars.forEach(star => {
			...
			else animateStaticStar(star, 1.8, rR(.5, 2))
		})

		planets.forEach(planet => {
			let scale;
			if(!isMobile()) scale = rR(1.5, 2.8)
			else scale = rR(1, 2)
			TweenMax.to(planet, .1, {opacity:rR(.5, 1)})
			TweenMax.to(planet, 1.8, {scale:scale, x:rR(-window.innerWidth/2, 0), y:r(window.innerHeight * .6)})
		})
